...
id_nl_compoundType: | id_nl compoundType { $2 }
id_nl             : | id NL? { $1 }
                    | LT NL? { "<" }
...
simpleExpr1       : | literal { $1 }
                    ...
                    | LPAREN exprs? RPAREN ARROW expr
                    {
                      EFun([
                        match $2 with
                        | None    -> []
                        | Some xs -> List.map (fun x -> (x, EUnit)) xs
                      ], EUnit, $5)
                    }
                    | LPAREN exprs? RPAREN
                    {
...
xmlExpr           : | xmllt id XML
                    {
                      match $3 with
                      | XmlTag(a,_,_) when a <> $2 ->
                        failwith "end tag error"
                      | _ -> EXml $3
                    }
                    | xmllt id XML_SINGLE
                    {
